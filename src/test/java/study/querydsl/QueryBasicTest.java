...
public class QueryBasicTest {
    ...
    /**
     * 회원과 팀을 join 하면서, 팀 이름이 teamA인 팀만 조회(회원은 모두 조회)
...
//                .on(team.name.eq("teamA"))
                .where(team.name.eq("teamA")) // inner join은 on절과 where에서 같은 결과
                .fetch();
    ...
    public void join_on_no_relation() {
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team) // id matching X -> 연관관계 없는 엔티티를 ON 절로 외부 조인
    ...
    public void basicCase() {
        List<String> result = queryFactory
                .select(
                        member.age
                                .when(10).then("ten years")
                                .when(20).then("twenty years")
                                .otherwise("others")
                )
                .from(member)
                .fetch();
    ...
    public void complexCase() {
        List<String> result = queryFactory
                .select(
                        new CaseBuilder()
                                .when(member.age.between(0, 20)).then("0~20 years")
                                .when(member.age.between(21, 30)).then("21~30 years")
                                .otherwise("others")
                )
                .from(member)
                .fetch();
